...
function App() {
  ...
  const getIntervalSeconds = (interval: string): number => {
    if (interval.endsWith('s')) {
      // Second-based interval (e.g., "5s", "15s", "30s")
      return parseInt(interval.slice(0, -1));
    }
    // Default to 60 seconds for cron expressions
    return 60;
  };
  ...
  useEffect(() => {
    ...
    const countdown = setInterval(() => {
      setNextUpdateIn(prev => {
        if (prev <= 1) {
          return intervalSeconds; // Reset to actual interval
        }
        return prev - 1;
      });
    }, 1000);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'price-update') {
        ...
        const intervalSeconds = settings ? getIntervalSeconds(settings.checkInterval) : 60;
        setNextUpdateIn(intervalSeconds); // Reset countdown
        setConnectionStatus('connected');
